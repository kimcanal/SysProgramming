...
void list_shuffle(struct list *list){
  ...
  //number of shuffles to operate
  int n = rand() % list_size(list);

  //prevent NO shuffle
  while(!n){
    n = rand() % list_size(list);
  }

 // printf("n is %d\n", n);
  while(n--){
    int index1 = rand() % list_size(list);
    int index2 =  rand() % list_size(list);
    //printf("INDEX: %d %d\n", index1, index2);
    struct list_elem *elem1 = get_list_elem_via_index(list, index1);
    struct list_elem *elem2 = get_list_elem_via_index(list, index2);

    //swap random elements element1 AND element2
    list_swap(elem1, elem2);                  
  }
  
}

struct list_elem* get_list_elem_via_index(struct list *LIST, int index) {
